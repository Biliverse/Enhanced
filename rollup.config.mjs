...
export function configAsset(suffix = "") {
	return {
		...
		async generateBundle() {
			...
			const source = `const response = {status: 200, headers: {"Content-Type":"application/json; charset=utf-8","X-PreferencePanes-Version":${JSON.stringify(version)}}, body: $request.method === "HEAD" ? "" : ${JSON.stringify(body)}};\n$done(typeof $task === "undefined" ? {response} : {...response, status:"HTTP/1.1 200 OK"});\n`;
